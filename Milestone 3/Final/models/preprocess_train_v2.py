...
def train_save_w2_model(df, embedding_file, desc_lines, w2v_model_dir):
	...
	x_train1, x_val, y_train1, y_val = train_test_split(X, y, test_size = 0.1, random_state = 1122, shuffle = True)

	print('##'*20, f'\nNumber of rows in training dataset: {x_train1.shape[0]}')
	print(f'Number of columns in training dataset: {x_train1.shape[1]}')
	print(f'Number of unique words in training dataset: {len(np.unique(np.hstack(x_train1)))}')

	print('##'*20, f'\nNumber of rows in validation dataset: {x_val.shape[0]}')
	print(f'Number of columns in validation dataset: {x_val.shape[1]}')
	print(f'Number of unique words in validation dataset: {len(np.unique(np.hstack(x_val)))}')
	print('##'*20)
	...
	print('\n Before Sampling: X_train: {}, y_train: {}'.format(x_train1.shape, y_train1.shape))
	print('\n After Sampling: X: {}, y: {}'.format(X_res.shape, y_res.shape))

	#Train-Test split of oversampled data:: ##TODO:
	print('\n Apply train-test split 80-20:\n\n')

	X_train2, X_test2, y_train2, y_test2 = train_test_split(X_res, y_res, test_size = 0.20, random_state = 1122)
	print('\n After oversampling: X_train: {}, y_train: {}'.format(X_train2.shape,y_train2.shape))
	print('\n After oversampling: X_test : {}, y_test : {}'.format(X_test2.shape,y_test2.shape))
	...
	h = model.fit(X_train2, y_train2, batch_size=64, epochs=10, validation_data=(x_val, y_val), verbose=2, shuffle=True)
	#h = model.fit(X_train2, y_train2, batch_size=32, epochs=10, validation_data=(X_test2, y_test2), verbose=2, shuffle=True)
	...
	score, acc = model.evaluate(X_test2, y_test2, batch_size=10)
	print('\nTest score:', score)
	print('\nTest accuracy:', acc)
	...
	#CR and CM::
	y_pred = model.predict_proba(X_test2, batch_size=64, verbose=1)
	y_pred_bool = np.argmax(y_pred, axis=1)
	y_test_bool = np.argmax(y_test2, axis=1)
	#y_pred = model.predict_proba(x_val, batch_size=32, verbose=1)
	#y_pred_bool = np.argmax(y_pred, axis=1)
	#y_test_bool = np.argmax(y_val, axis=1)
...
def predict(input_str, w2v_model_dir, w2v_embedding_file):
	...
	ip_padded = desc_pad
	y_pred = loaded_model.predict_proba(ip_padded, batch_size=64, verbose=1)
